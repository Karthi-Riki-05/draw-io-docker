...
(function () {
	...
	EditorUi.prototype.generateOpenAiMermaidDiagram = function (prompt, success, error, enableParser, options) {
		...
		var fn = mxUtils.bind(this, function () {
			this.createTimeout(this.editor.generateTimeout, mxUtils.bind(this, function (timeout) {
				...
				// Use configurable AI endpoint (defaults to local proxy)
				var url = window.DRAWIO_AI_URL || 'https://www.draw.io/generate/v3';

				// DEBUG: Print the query before sending to backend
	...
	EditorUi.prototype.loadMermaid = function (success, error) {
		...
		if (typeof mermaid === 'undefined' && !this.loadingMermaid && !this.isOffline(true)) {
			...
			// Always load extensions.min.js - it contains mermaid bundled with proper hooks
			// that connect to mxMermaidToDrawio for native shape conversion.
			// Loading a separate mermaid.min.js breaks the hooks.
